...
function sortedArrayToBST($nums)
{
	...
	if ($middle_index - 1 >= 0) {
		...
		$left_side = sortedArrayToBST(array_slice($nums, 0, $middle_index));
	};
	if ($middle_index + 1 < count($nums)) {

		$right_side =sortedArrayToBST(array_slice($nums, $middle_index + 1));
	};
...
var_dump(sortedArrayToBST([-1, 0, 1, 2]));
